                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 max-h-48 overflow-y-auto pr-2">
                      {slots.map(slot => {
                        ...
                        return (
                          <button key={slot} type="button" disabled={isBooked} onClick={() => setSelectedSlot(slot)} className={`p-3 text-sm rounded-lg font-medium transition-all ${selectedSlot === slot ? 'bg-slate-800 text-white' : isBooked ? 'bg-slate-100 text-slate-300 cursor-not-allowed line-through' : 'bg-slate-50 text-slate-700 hover:bg-slate-200'}`}>{slot}</button>
                        );
                      })}
                    </div>
                  </div>
